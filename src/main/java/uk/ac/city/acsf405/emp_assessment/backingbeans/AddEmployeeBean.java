/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package uk.ac.city.acsf405.emp_assessment.backingbeans;

import java.util.List;
import javax.ejb.EJB;
import javax.faces.bean.ManagedBean;
import uk.ac.city.acsf405.emp_assessment.domain.*;
import uk.ac.city.acsf405.emp_assessment.services.EmployeeManagementService;

/**
 *
 * @author Elmar Talibzade
 */

@ManagedBean (name="addEmployeeBean")
public class AddEmployeeBean 
{
    @EJB
    private EmployeeManagementService service;
    private List<Employee> employees;
    
    public String addEmployee(Short ID, Short departmentID, String name, String job)
    {
        Employee newEmployee = new Employee();
        newEmployee.setEmployeeID(ID);
        newEmployee.setDepartmentID(new Department(departmentID));
        newEmployee.setName(name);
        newEmployee.setJob(job);
        
        employees = service.addEmployee(newEmployee);
        return "allEmployees";
    }
}